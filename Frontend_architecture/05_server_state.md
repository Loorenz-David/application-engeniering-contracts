...
If the data comes from the backend, it lives in TanStack Query. Every query and mutation is defined as a hook inside `features/<feature>/api/`.
...
Create uses a `client_id` — a UUID generated by the frontend before the request fires. The backend stores the entity with `id = client_id`. This means the ID is known upfront, so the detail cache can be seeded immediately and navigation to the new entity is instant. See [24_dto.md](24_dto.md) for the full `client_id` convention.
...
When one query depends on another, use the `enabled` flag:

```ts
const { data: workspace } = useWorkspaceQuery();

const { data: members } = useQuery({
  queryKey: memberKeys.list(workspace?.id),
  queryFn: () => fetchMembers(workspace!.id),
  enabled: Boolean(workspace?.id),  // only runs after workspace loads
});
```
