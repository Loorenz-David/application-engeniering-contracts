...
class <Entity>HistoryRecord(IdentityMixin, HistoryRecord, db.Model):
    ...
    <entity>_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("<entity_table>.id"), nullable=False, index=True
    )
...
`updated_by` must declare `foreign_keys` explicitly because every history table has two FKs pointing at `users.id` (`<entity>_id` goes to the parent entity; `updated_by_id` goes to User).
...
class <Entity>(IdentityMixin, db.Model):
    ...
    last_history_record_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("<entity>_history_records.id"), nullable=True
    )
...
class Customer(IdentityMixin, db.Model):
    ...
    last_history_record_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("customer_history_records.id"), nullable=True
    )
...
def update_customer(ctx: ServiceContext) -> dict:
    ...
    customer.last_history_record_id = history.id # update the pointer

    db.session.commit()
    return {"customer_id": customer.client_id}
```

`db.session.flush()` before setting `last_history_record_id` is required — `flush()` assigns the database-generated `id` to `history` without committing the transaction. The pointer and the row are committed atomically.
...
- **`flush()` before setting `last_history_record_id`.** `flush()` assigns the DB-generated `id` without committing. The pointer and the history row commit atomically.
