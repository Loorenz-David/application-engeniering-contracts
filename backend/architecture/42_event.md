...
class <Entity>(IdentityMixin, db.Model):
    ...
    last_event_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("<entity>_events.id"), nullable=True
    )
...
class Task(IdentityMixin, db.Model):
    ...
    last_event_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("task_events.id"), nullable=True
    )
...
Command
  ...
  │        flush() → get event.id
  │
  ├─ 2. UPDATE task.last_event_id = event.id
  │
...
- **`last_event_id` is updated in the same transaction as the `INSERT`.** Use `flush()` to get the event `id` before committing.
