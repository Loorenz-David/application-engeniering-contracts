...
class ContentMentionLink(IdentityMixin, db.Model):
    ...
    content_mention_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("content_mentions.id"), nullable=False, index=True
    )
...
No DB-level FK on `entity_id` — same polymorphic pattern as `CaseLink` and `ImageLink`. The entity type uniquely identifies which table `entity_id` refers to.
...
def send_message(ctx) -> dict:
    ...
    db.session.flush()                  # get message.id before processing mentions
...
The `validate_content` call must come before the DB write. The `process_content_mentions` call must come after `flush()` so `entity_id` (the message's internal `id`) is available.
...
- **`process_content_mentions` must be called after `flush()`.** The `entity_id` argument requires the row's internal `id`, which is only assigned after the session is flushed.
...
- **`ContentMentionLink.entity_id` has no DB-level FK.** Same polymorphic pattern as `CaseLink` and `ImageLink` — the handler layer is responsible for ensuring the entity exists.
- **`ContentMention.mention_id` stores the `client_id` string, not the internal integer `id`.** It is the value from `mention.client_id` in the input content block.
