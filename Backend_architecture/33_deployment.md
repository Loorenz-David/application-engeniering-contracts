...
- [ ] `flask db current` matches the expected migration head on staging
...
```
1. Run: flask db upgrade HEAD        (apply pending migrations — additive only)
...
3. Run: flask db upgrade HEAD        (drop the column)
4. Monitor
```
...
2. Leave the migration in place — do NOT run flask db downgrade
3. The new column is unused but harmless
4. Fix the issue in the new version and redeploy
```

**Never run `flask db downgrade` in production unless:**
...
if current_app.config.get("FEATURE_NEW_CHECKOUT_FLOW"):
