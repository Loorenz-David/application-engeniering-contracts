...
# 2. Generate
flask db migrate -m "create_<entity>_table"
...
def create_entity():
    ...
    return run_service(create_entity_command, ctx)
```

Register the blueprint in `routers/api_v1/__init__.py`.
...
- [ ] Migration applied (`flask db upgrade`) without errors
...
Use this workflow when you are starting a new Flask application from scratch using this contract as its foundation.
...
- Load config by `FLASK_ENV`
- Init SQLAlchemy, Migrate, JWT, CORS, Socket.IO
- Register blueprints
...
| Writing migration SQL by hand | Type mismatches, missing indexes, broken downgrade | Always auto-generate with `flask db migrate` |
