...
Generate and attach the ID in the `before_request` middleware:
...
`getattr(g, "request_id", "-")` handles the case where logging fires outside a request context (e.g., background jobs). Background jobs should generate their own correlation ID at job entry:
