...
**Never touch the database manually.** No `ALTER TABLE` in psql. No column drops via a DB GUI. No `db.create_all()` in production. Every schema change goes through an Alembic migration that is committed to the repository, reviewed, and applied with `flask db upgrade`. No exceptions.
...
- Seeding reference data in development (use `flask seed-dev-data`)
...
# 2. Generate
flask db migrate -m "add_status_to_records"
...
flask db migrate -m "add_status_to_records"
flask db migrate -m "add_workspace_id_to_categories"
flask db migrate -m "create_record_attachments_table"
flask db migrate -m "drop_legacy_active_flag_from_users"

# Bad — no context
flask db migrate -m "update"
flask db migrate -m "fix"
flask db migrate -m "migration_1"
```
...
Before running `flask db upgrade`, open the generated file and verify every item in this checklist:
...
```bash
flask backfill-record-status --dry-run
flask backfill-record-status
```
...
1. Run `flask db current` to confirm your local DB is at the expected head.
2. Run `flask db history` to verify the chain is linear (no branches).
3. If the chain is branched, merge with `flask db merge <rev1> <rev2>`.

```bash
flask db current          # current revision on this DB
flask db history          # full chain
flask db show <revision>  # inspect a specific migration
```
...
Run this before every `flask db upgrade` in production:
